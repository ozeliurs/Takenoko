...
class ColletBotTest {
    ...
    class IntegrationTests {
        @Test
        @DisplayName("The first two movements of the bot should therefore be to take an objective card and take an irrigation canal.")
        void theFirstTwoMovementsOfTheBotShouldThereforeBeToTakeAnObjectiveCardAndTakeAnIrrigationCanal() {
            Board board = new Board();
            BotState botState = spy(BotState.class);
            History history = mock(History.class);
            ColletBot colletBot = new ColletBot();

            when(botState.getAvailableActions()).thenReturn(
                    List.of(
